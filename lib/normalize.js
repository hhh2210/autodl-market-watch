...
function filterRows(rows, { regions = [], maxPrice = null } = {}) {
  const normalizedRegions = regions.map((value) => value.toLowerCase());
  return rows.filter((row) => {
    const region = `${row.region} ${row.raw.region_sign || ""}`.toLowerCase();
    const regionMatches =
      normalizedRegions.length === 0 ||
      normalizedRegions.some((candidate) => region.includes(candidate));
    const price = Number(row.pricePerHour);
    const priceMatches =
      maxPrice === null || (Number.isFinite(price) && price <= maxPrice);
    return regionMatches && priceMatches;
  });
}

module.exports = { normalizeRow, dedupeRows, filterRows };
